...
def run_simulation(
    process=0, num_episodes=500, return_dict=None, seed_number=0, base_vel_command_type="human", render=True
):
        ...
        for _ in range(N_STEPS_PER_EPISODE):
            ...
            if env.step_num >= N_STEPS_PER_EPISODE or is_terminated or is_truncated:
                ...
                if return_dict is not None:
                    return_dict['process' + str(process) + '_ctrl_state_history_ep' + str(episode_num)] = np.array(
                        ctrl_state_history
                    ).reshape(-1, len(ctrl_state))
