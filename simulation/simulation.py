...
def run_simulation(
    ...
    if recording_path is not None:
        ...
        assert root_path.exists(), f"Recording path does not exist: {root_path.absolute()}"
        dataset_path = (
            root_path
    ...
    for episode_num in range(N_EPISODES):
        ...
        for _ in tqdm(range(N_STEPS_PER_EPISODE), desc=f"Ep:{episode_num:d}-steps:", total=N_STEPS_PER_EPISODE):
            ...
            # ctrl_state["base_lin_vel_err"] = base_lin_vel_err
            # ctrl_state["base_ang_vel_err"] = base_ang_vel_err
