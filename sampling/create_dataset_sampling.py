...
if(config.robot == 'go2'):
    m = mujoco.MjModel.from_xml_path(dir_path + '/../simulation/robot_model/go2/scene_' + scene  + '.xml')
elif(config.robot == 'aliengo'):
    m = mujoco.MjModel.from_xml_path(dir_path + '/../simulation/robot_model/aliengo/scene_' + scene  + '.xml')
elif(config.robot == 'hyqreal'):
    m = mujoco.MjModel.from_xml_path(dir_path + '/../simulation/robot_model/hyqreal/scene_' + scene  + '.xml')
elif(config.robot == 'mini_cheetah'):
    m = mujoco.MjModel.from_xml_path(dir_path + '/../simulation/robot_model/mini_cheetah/scene_' + scene  + '.xml')
else:
...
# Data collection
maximum_step = 100000
time_to_wait_for_saving = maximum_step - 10
time_to_wait_for_disturbance = 1000
time_to_wait_for_new_velocity = 1000

data_state = []
data_reference = []
data_input = []
data_external_disturbance = []
disturbance_wrench = np.zeros((6,))
start_disturbance_boolean = 0
duration_disturbance = 500
...
with mujoco.viewer.launch_passive(m, d, show_left_ui=False, show_right_ui=False) as viewer:
    ...
    while i < maximum_step:
        ...
        if(i > time_to_wait_for_saving):
            with open('data_estimator.npy', 'wb') as f:
                #save!
                np.save(f, data_state)
                np.save(f, data_reference)
                np.save(f, data_input)
                np.save(f, data_external_disturbance)
        ...
        if(i > time_to_wait_for_disturbance):
            ref_linear_velocity = np.array([np.random.uniform(0.1, 0.8), 0.0, 0.0])
            time_to_wait_for_disturbance += np.random.uniform(500, 1500)
        ...
        if(config.simulation_params['use_external_disturbances']):
            #if(i % duration_disturbance == 0 and i > time_to_wait_for_disturbance):
            if(i > time_to_wait_for_disturbance):
                ...
                disturbance_wrench = np.array([np.random.uniform(-disturbance_wrench_bound[0], disturbance_wrench_bound[0]),
                                            np.random.uniform(-disturbance_wrench_bound[1], disturbance_wrench_bound[1]),
                                            np.random.uniform(-disturbance_wrench_bound[2], disturbance_wrench_bound[2]),
                                            np.random.uniform(-disturbance_wrench_bound[3], disturbance_wrench_bound[3]),
                                            np.random.uniform(-disturbance_wrench_bound[4], disturbance_wrench_bound[4]),
                                            np.random.uniform(-disturbance_wrench_bound[5], disturbance_wrench_bound[5])])
                
                #duration_disturbance = np.random.choice([250, 500, 1000])
                disturbance_lenght = np.random.uniform(50, 500)
                duration_disturbance = i + disturbance_lenght
                start_disturbance_boolean = 1
                #time_to_wait_for_disturbance = i + 1000
                disturbance_sleep = np.random.uniform(500, 1000)
                time_to_wait_for_disturbance = i + disturbance_lenght + disturbance_sleep
                print("disturbance START")
                print("with length: ", disturbance_lenght)
                print("and a sleep of: ", disturbance_sleep)
                print("the disturbance will be off at step: ", duration_disturbance)
                print("and the next disturbance will be at step: ", time_to_wait_for_disturbance)
            elif(i > duration_disturbance):
                disturbance_wrench = np.zeros((6,))
                start_disturbance_boolean = 1
                print("disturbance OFF")
            else:
                print("disturbance ON")
                start_disturbance_boolean = 0

            d.qfrc_applied[0] = disturbance_wrench[0]
            d.qfrc_applied[1] = disturbance_wrench[1]
            d.qfrc_applied[2] = disturbance_wrench[2]
            d.qfrc_applied[3] = disturbance_wrench[3]
            d.qfrc_applied[4] = disturbance_wrench[4]
            d.qfrc_applied[5] = disturbance_wrench[5]
        ...
        if(i>1000):
            ...
            temp_state[0, 49] = optimize_swing 
            temp_state[0, 50] = current_contact[0]
            temp_state[0, 51] = current_contact[1]
            temp_state[0, 52] = current_contact[2]
            temp_state[0, 53] = current_contact[3]
            temp_state[0, 54] = swing_time[0]/swing_period
            temp_state[0, 55] = swing_time[1]/swing_period
            temp_state[0, 56] = swing_time[2]/swing_period
            temp_state[0, 57] = swing_time[3]/swing_period
            ...
            temp_ref[0, 9:12] = nmpc_footholds[0]
            temp_ref[0, 12:15] = nmpc_footholds[1]
            temp_ref[0, 15:18] = nmpc_footholds[2]
            temp_ref[0, 18:21] = nmpc_footholds[3]
            temp_ref[0, 21] = 1.3
            ...
            print("step_freq: ", pgg.step_freq)
            print("disturbance_wrench: ", disturbance_wrench)