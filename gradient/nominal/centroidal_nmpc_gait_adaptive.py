...
class Acados_NMPC_GaitAdaptive():
    def __init__(self):
        ...
        self.ocp = self.create_ocp_solver_description(acados_model, num_threads_in_batch_solve = len(config.mpc_params['step_freq_available']))
        ...
        self.batch_solver = AcadosOcpBatchSolver(batch_ocp, self.batch, verbose=False, json_file=self.ocp.code_export_directory + "/centroidal_nmpc_batch" + ".json")
    ...
    def compute_batch_control(self, state, reference, contact_sequence, constraint = None, external_wrenches = np.zeros((6,)), 
                         inertia = config.inertia.reshape((9,)), mass = config.mass):
        start = time.time()
        ...
        for n in range(self.batch):
            ...
            if(n != 0):
                cost_single_qp +=  3*(config.mpc_params["step_freq_available"][n] - config.mpc_params["step_freq_available"][0])**2
