...
class InverseKinematicsAdam:
    def __init__(self, urdf_filename, xml_filename) -> None:
        """
        ...
        if(self.use_adam):
            ...
            model_idyntree = idyntree.ModelLoader()
            model_idyntree.loadReducedModelFromFile(urdf_filename, joint_list)
            self.kindyn_idyntree = idyntree.KinDynComputations()
            self.kindyn_idyntree.loadRobotModel(model_idyntree.model())
            self.kindyn_idyntree.setFrameVelocityRepresentation(idyntree.MIXED_REPRESENTATION)
...
if __name__ == "__main__":
    ...
    ik = InverseKinematicsAdam(urdf_filename=urdf_filename, xml_filename=xml_filename)   
