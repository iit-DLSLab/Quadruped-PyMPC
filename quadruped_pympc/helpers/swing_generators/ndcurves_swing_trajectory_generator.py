...
class SwingTrajectoryGenerator:
    def __init__(self, step_height: float, swing_period: float) -> None:
        ...
        self._constraints.init_vel = np.array([[0.0, 0.0, 0.0]]).T
        self._constraints.end_vel = np.array([[0.0, 0.0, 0.0]]).T
        self._constraints.init_acc = np.array([[0.0, 0.0, 0.0]]).T
        self._constraints.end_acc = np.array([[0.0, 0.0, 0.0]]).T

    def createBezierCurve(self, x0, xf):
        scaling_factor = 0.7105
        p1 = x0 + np.array([0.0, 0.0, self._stepHeight / scaling_factor])
        p2 = 0.5 * (x0 + xf) + np.array([0.0, 0.0, self._stepHeight / scaling_factor])
        p3 = xf + np.array([0.0, 0.0, self._stepHeight / scaling_factor])
        self._curve = ndcurves.bezier(
            np.array([x0, p1, p2, p3, xf]).T, self._constraints, 0.0, (self._N - 1) * self._dt
        )
...
if __name__ == "__main__":
    ...
    trajectory_generator = SwingTrajectoryGenerator(
        step_height=step_height, swing_period=swing_period, position_gain_fb=0, velocity_gain_fb=0
    )
    ...
    for foot_swing_time in np.arange(0.000001, swing_period, 0.002):
        desired_foot_position, desired_foot_velocity, _ = trajectory_generator.compute_trajectory_references(
            i, np.array([0, 0, 0]), np.array([0.3, 0, 0])
        )
        i += 1
