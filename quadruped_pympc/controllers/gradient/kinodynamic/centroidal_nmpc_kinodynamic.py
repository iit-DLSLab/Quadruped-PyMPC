...
from .centroidal_model_kinodynamic import Centroidal_Model_KinoDynamic
import numpy as np
import scipy.linalg
import casadi as cs
import copy
...
class Acados_NMPC_KinoDynamic:
    def __init__(self):
        ...
        self.centroidal_model = Centroidal_Model_KinoDynamic()
    ...
    def set_weight(self, nx, nu):
        ...
        R_joint_vel = np.array([0.01, 0.01, 0.01]) 
    ...
    def compute_control(self, state, reference, contact_sequence, constraint = None, external_wrenches = np.zeros((6,)), 
        ...
        R_from_euler = SO3.from_euler(np.array([roll, pitch, yaw])).as_matrix()

        quaternion = state["quaternion"]
        quaternion = np.array([quaternion[1], quaternion[2], quaternion[3], quaternion[0]])
        R_from_quaternion = SO3.from_quat(quaternion).as_matrix()

        print("R_from_euler: ", R_from_euler)
        print("R_from_quaternion: ", R_from_quaternion)
        print("b_R_w.T: ", b_R_w.T)
        print("b_R_w: ", b_R_w)
