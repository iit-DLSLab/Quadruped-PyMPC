...
class Acados_NMPC_KinoDynamic_Batched:
    def __init__(self):
        ...
        batch_ocp = self.ocp
        dir_path = os.path.dirname(os.path.realpath(__file__))
        batch_ocp.code_export_directory = dir_path + '/c_generated_code_batched'
        self.batch_solver = AcadosOcpBatchSolver(batch_ocp, self.batch, verbose=False,
                                                 json_file=self.ocp.code_export_directory + "/kynodinamic_nmpc_batch"
                                                           + ".json")
    ...
    def compute_batch_control(self, state_list, reference_list, contact_sequence_list, constraint = None, external_wrenches = np.zeros((6,)), 
        ...
        for n in range(self.batch):
            ...
            contact_sequence = contact_sequence_list[n]
            state = state_list[n]
