...
class Sampling_MPC:
    ...
    def __init__(self, horizon = 200, dt = 0.01, num_parallel_computations = 10000, sampling_method = 'random_sampling', control_parametrization = "linear_spline_1", device="gpu",controller_params = None):
        """
        Args:
        ...
        if(sampling_method == 'random_sampling'):
            ...
            self.sigma_random_sampling = self.mpc_params['sigma_random_sampling']
        elif(sampling_method == 'mppi'):
            self.compute_control = self.compute_control_mppi    
            self.sigma_mppi = self.mpc_params['sigma_mppi']
        elif(sampling_method == 'cem_mppi'):
            self.compute_control = self.compute_control_cem_mppi
            self.sigma_cem_mppi = jnp.ones(self.num_control_parameters, dtype=dtype_general) * self.mpc_params['sigma_cem_mppi']
        else:
        ...
        self.mu = self.mpc_params['mu']

        # maximum allowed z contact forces 
        self.f_z_max = self.mpc_params['grf_max']
        self.f_z_min = self.mpc_params['grf_min']
    ...
    def prepare_state_and_reference(self, state_current, reference_state, 
        ...
        if (self.mpc_params['shift_solution']):
