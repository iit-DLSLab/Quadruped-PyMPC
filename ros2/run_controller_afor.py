...
USE_SATURATED_LOOP_TIME = (
    False  # This is used to cap the clock time of periodic gait gen to max 250Hz
)
...
class Quadruped_PyMPC_Node:
    def __init__(self, session: afor.BaseSession | None = None):
        self.ros_session = afor.auto_session(session)

        # Publishers are standard ROS 2 publishers created through the afor session.
        with self.ros_session.lock() as node:
            self.ros_node = node
            self.publisher_time_debug = node.create_publisher(TimeDebug, "/time_debug", 1)
            self.publisher_control_signal = node.create_publisher(
                ControlSignal, "/control_signal", 1
            )
        ...
        if USE_PROCESS_QUEUE_MPC:
            ...
            process_mpc = multiprocessing.Process(
                target=self.compute_mpc_process_queue_callback,
    ...
    def compute_mpc_process_queue_callback(
        self, input_data_process, output_data_process
    ):
        pid = os.getpid()
        os.system("sudo renice -n -21 -p " + str(pid))
        os.system("sudo echo -20 > /proc/" + str(pid) + "/autogroup")
    ...
    def compute_mpc_process_shared_memory_callback(
        self, input_data_process, shm_out_name: str, seq_out: Value
    ):
        pid = os.getpid()
        os.system("sudo renice -n -21 -p " + str(pid))
        os.system("sudo echo -20 > /proc/" + str(pid) + "/autogroup")
        ...
        while True:
            ...
            if not input_data_process.empty():
                ...
                arr[IDX_JP] = (
                    nmpc_joints_pos
                    if nmpc_predicted_state is not None
                    else np.zeros(12).reshape(-1)[:12]
                )
                arr[IDX_JV] = (
                    nmpc_joints_pos
                    if nmpc_predicted_state is not None
    ...
    def get_base_state_callback(self, msg):
        ...
        if USE_SMOOTH_HEIGHT:
            ...
            self.position[2] = (
                0.5 * self.position[2] + 0.5 * np.array(msg.pose.position)[2]
            )
        else:
    ...
    def get_joy_callback(self, msg):
        ...
        if msg.buttons[8] == 1:
            self.ros_node.get_logger().info(
                "Joystick button pressed, shutting down the node."
            )
    ...
    def compute_control_callback(self):
        ...
        if USE_FIXED_LOOP_TIME:
            ...
            if USE_SATURATED_LOOP_TIME:
                # Cap raised 0.005 -> 0.013: with the inline MPC the loop runs
                # ~4.5 ms (WBC only) but ~10 ms on MPC-solve ticks. A 5 ms cap
                # clipped those ~10 ms ticks, so the gait/swing clock advanced at
                # only ~75% of real time -> systematic touchdown-phase desync.
                # 13 ms sits above the normal loop time so the cap now only
                # catches genuine multi-tick stalls.
                if simulation_dt > 0.013:
                    simulation_dt = 0.013
        ...
        if USE_THREADED_MPC:
            ...
            if self.shm_out is not None and self.seq_out is not None:
                ...
                if s1 % 2 == 0:  # writer not in progress
                    buf = np.ndarray(
                        (N_DBL,), dtype=np.float64, buffer=self.shm_out.buf
                    )
        ...
        control_signal_msg.timestamp = float(
            self.ros_node.get_clock().now().nanoseconds
        )
        control_signal_msg.joints_torques = (
            np.concatenate([self.tau.FL, self.tau.FR, self.tau.RL, self.tau.RR], axis=0)
            .flatten()
            .tolist()
        )
        ...
        control_signal_msg.joints_velocity = (
            np.concatenate(
                [
...
async def base_state_sub_coro(controller_node: Quadruped_PyMPC_Node):
    base_state_sub = afor.Sub(BaseState, "/base_state", QOS_DEPTH_1)
    async for msg in base_state_sub.listen():
        controller_node.get_base_state_callback(msg)


async def blind_state_sub_coro(controller_node: Quadruped_PyMPC_Node):
    blind_state_sub = afor.Sub(BlindState, "/blind_state", QOS_DEPTH_1)
    async for msg in blind_state_sub.listen():
        controller_node.get_blind_state_callback(msg)


async def joy_sub_coro(controller_node: Quadruped_PyMPC_Node):
    joy_sub = afor.Sub(Joy, "joy", QOS_DEPTH_1)
    async for msg in joy_sub.listen():
        controller_node.get_joy_callback(msg)


async def scheduler_coro(controller_node: Quadruped_PyMPC_Node):
    rate = afor.Rate(SCHEDULER_FREQ)
    async for _ in rate.listen():
        controller_node.compute_control_callback()


@afor.scoped
async def run_controller():
    controller_node = Quadruped_PyMPC_Node()
    task_group = afor.Scope.current().task_group
    assert task_group is not None
    task_group.create_task(base_state_sub_coro(controller_node), name="base_state_sub")
    task_group.create_task(blind_state_sub_coro(controller_node), name="blind_state_sub")
    task_group.create_task(joy_sub_coro(controller_node), name="joy_sub")
    if USE_SCHEDULER:
        task_group.create_task(scheduler_coro(controller_node), name="control_scheduler")
    await asyncio.Future()


def main():
    print("Hello from Quadruped-PyMPC ros interface.")
    with afor.auto_context("Quadruped_PyMPC_Node"):
